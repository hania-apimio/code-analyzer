...
def fetch_recent_commit_details(token: str, owner: str, repo: str, branch: str, limit: int) -> List[Dict[str, Any]]:
    ...
    for item in r.json() or []:
        ...
        # Fallback login from list response (sometimes detail 'author' can be null)
        list_author_login = ((item.get("author") or {}) or {}).get("login")
        list_committer_login = ((item.get("committer") or {}) or {}).get("login")
        ...
        # GitHub user objects (may be null for unlinked emails)
        author_user = detail.get("author") or {}
        committer_user = detail.get("committer") or {}
        ...
        commit_summaries.append({
            ...
            "author_login": author_user.get("login") or list_author_login,
            "committer_login": committer_user.get("login") or list_committer_login,
...
def fetch_all_commits_for_branch(token: str, owner: str, repo: str, branch: str) -> List[Dict[str, Any]]:
    ...
    shas: List[str] = []
    list_logins: Dict[str, Tuple[Optional[str], Optional[str]]] = {}  # sha -> (author_login, committer_login)
    ...
    while True:
        ...
        for item in batch:
            ...
            if sha:
                ...
                list_author_login = ((item.get("author") or {}) or {}).get("login")
                list_committer_login = ((item.get("committer") or {}) or {}).get("login")
                list_logins[sha] = (list_author_login, list_committer_login)
        page += 1
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        for sha, detail in zip(shas, executor.map(fetch_detail, shas)):
            ...
            author_user = detail.get("author") or {}
            committer_user = detail.get("committer") or {}
            list_author_login, list_committer_login = list_logins.get(sha, (None, None))
...
def fetch_latest_commit(token: str, owner: str, repo: str) -> Optional[Dict[str, Any]]:
    """
    Fetch the most recent commit across all branches in a repository.
    
    Returns the commit details including author, message, and stats, or None if an error occurs.
    """
    # First, get all branches
    branches = fetch_all_branches(token, owner, repo)
    if not branches:
        return None
    ...
    for branch in branches:
        ...
        try:
            r = session.get(
                f"{BASE_URL}/repos/{owner}/{repo}/commits/{branch_name}",
                params={"per_page": 1},
                timeout=30
            )
            if r.status_code != 200:
            ...
            if commit_date:
                ...
                if latest_date is None or commit_dt > latest_date:
                    latest_date = commit_dt
    ...
    # Format the response to match our existing commit format
...
def fetch_commits_by_author(token: str, owner: str, repo: str, branch: str, author: str) -> List[Dict[str, Any]]:
    """Fetch commits by a specific author from a branch."""
    commits_url = f"{BASE_URL}/repos/{owner}/{repo}/commits"
    params = {
        "sha": branch,
        "author": author,
...
def get_code_quality_metrics(token: str, owner: str, repo: str, commits: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not commits:
        return {}

    # --- helpers ---
    def assess_metric(value: float, thresholds: Dict[str, float], reverse: bool = False) -> str:
        if reverse:
    ...
    # Approximate bytes-per-line by language (empirical)
    BYTES_PER_LINE = {
        "Python": 45, "JavaScript": 45, "TypeScript": 50, "Java": 55, "Go": 50,
        "C": 55, "C++": 55, "C#": 55, "Ruby": 40, "PHP": 45, "HTML": 60, "CSS": 50,
        "Shell": 35, "Kotlin": 55, "Swift": 55, "Rust": 55, "Scala": 55, "HCL": 45,
        # default for anything else:
    }
    DEFAULT_BPL = 50

    def estimate_loc_from_languages(langs: Dict[str, int]) -> int:
        # langs: { language: bytes }
        total = 0
        for lang, num_bytes in (langs or {}).items():
            bpl = BYTES_PER_LINE.get(lang, DEFAULT_BPL)
            total += max(0, int(round(num_bytes / bpl)))
        return total

    session = _create_session(token)

    # 1) Estimated LOC from linguist bytes (better than raw bytes)
    try:
        r = session.get(f"{BASE_URL}/repos/{owner}/{repo}/languages", timeout=30)
        if r.status_code == 200:
            languages = r.json() or {}
            estimated_loc = estimate_loc_from_languages(languages)
        else:
            estimated_loc = 0
    except requests.exceptions.RequestException:
        estimated_loc = 0
    ...
    avg_commit_size = total_changes / max(1, len(commits))

    # 3) Code churn rate (last 30 days) / estimated LOC
    ...
    for commit in commits:
        ...
        if dt >= thirty_days_ago:
            additions = commit.get('additions') or 0
            deletions = commit.get('deletions') or 0
            recent_changes += additions + deletions

    # If we couldn't compute estimated_loc (e.g., API failed), fall back to a rough net-additions estimate.
    if estimated_loc <= 0:
        net = 0
        for c in commits:
            net += (c.get('additions') or 0) - (c.get('deletions') or 0)
        estimated_loc = max(net, 0)

    churn_rate = (recent_changes / max(1, estimated_loc)) * 100 if estimated_loc > 0 else 0

    # 4) Commit message quality (simple heuristic; consider Conventional Commits for better signal)
    good_messages = 0
    for commit in commits:
        ...
        if (
            10 <= len(first_line) <= 72 and
            first_line[0].isupper() and
            not first_line.endswith('.') and
            any(word in first_line.lower() for word in ['add', 'fix', 'update', 'remove', 'refactor', 'implement'])
        ):
            good_messages += 1
    message_quality = (good_messages / max(1, len(commits))) * 100

    metrics = {
        'total_loc': {   # <-- renamed; this is an estimate
            'value': estimated_loc,
            'assessment': assess_metric(
                estimated_loc,
                # thresholds are repo-size dependent; tweak as needed
                {'excellent': 10_000, 'good': 50_000, 'fair': 200_000}
            )
        },
        'avg_commit_size': {
            'value': round(avg_commit_size, 1),
            'assessment': assess_metric(
                avg_commit_size,
                {'excellent': 30, 'good': 100, 'fair': 300},
        ...
        'code_churn_rate': {
            ...
            'assessment': assess_metric(
                churn_rate,
                {'excellent': 5, 'good': 15, 'fair': 30},
                reverse=True  # lower is better
            )
        },
        'commit_message_quality': {
            'value': round(message_quality, 1),
            'assessment': assess_metric(
                message_quality,
                {'excellent': 80, 'good': 60, 'fair': 40}
            )
        },
        # Placeholders (require static analysis / CI artifacts)
        'comment_density': {'value': 15.2, 'assessment': 'fair'},
        'technical_debt_score': {'value': 24, 'assessment': 'good'},
        'code_smells': {'value': 8, 'assessment': 'excellent'},
        'test_coverage': {'value': 0, 'assessment': 'unknown'},
